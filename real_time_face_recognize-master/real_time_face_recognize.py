...
parser.add_argument("--videopath", type=str,default ="/home/leonard/face_detection/video/2017届毕业典礼（上）.mp4")
...
def add_chinese(img,name,text_position):
    ...
    # 字体  
    font = ImageFont.truetype("/usr/share/fonts/windows/fonts/arial.ttf", 20, encoding="utf-8")
...
# 先获取视频文件的信息，json格式
VIDEO_PROBE = getVideoProbeInfo(args.videopath)
# 得到视频文件的时长，秒为单位
...
with tf.Graph().as_default():
        with tf.Session() as sess:
            ...
            video_capture = cv2.VideoCapture(args.videopath)
                    ...
                    if len(bounding_boxes) < 1:
                        ...
                        for item,face_position in enumerate(bounding_boxes):                        
                            ...
                            if min(dist) > 0.7:
                                ...
                                print("未收录入人脸识别库")
                                dist = [] 
                                frame = add_chinese(frame,"Not",(int(face_position[0]), int(face_position[1]-30)))
                            else:    
                                #print(min(dist))
